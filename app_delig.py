...
class App:
    ...
    def on_double_click(self, event):
        """Обрабатывает двойной клик для inline-редактирования"""
        # Получаем элемент под курсором
        region = self.table.identify("region", event.x, event.y)
        if region != "cell":
            return
    
        # Получаем ID элемента и колонку
        self.edit_item = self.table.identify_row(event.y)
        column = self.table.identify_column(event.x)
    
        if not self.edit_item or not column:
            return
    
        # Получаем ID колонки (например, #3)
        self.edit_column_id = column
    
        # Получаем значение ячейки
        item_values = self.table.item(self.edit_item, "values")
        col_index = int(column.replace("#", "")) - 1
        current_value = item_values[col_index] if col_index < len(item_values) else ""
    
        # Для таблицы workload редактирование только группы (колонка #4)
        if self.current_table == "workload":
            if column != "#4":  # Только колонка "Группа"
                self.edit_record()  # Открываем окно редактирования
                return
    
        # Создаём Entry для редактирования
        self.edit_entry = tk.Entry(self.table, relief=tk.FLAT, bg="white", selectbackground="lightblue")
        self.edit_entry.insert(0, current_value)
        self.edit_entry.select_range(0, tk.END)
    
        # Получаем координаты ячейки
        bbox = self.table.bbox(self.edit_item, column)
        if bbox:
            ...
            self.edit_entry.bind("<FocusOut>", self.save_inline_edit)
    ...
    def save_inline_edit(self, event=None):
        ...
        item_values = list(self.table.item(self.edit_item, "values"))
        col_index = int(self.edit_column_id.replace("#", "")) - 1
        old_value = item_values[col_index]
    
        # Если значение не изменилось — просто закрываем
        ...
        # Валидация в зависимости от таблицы и колонки
        if not self.validate_inline_edit(new_value, col_index):
            messagebox.showwarning("Ошибка", "Некорректное значение!")
            self.edit_entry.destroy()
            self.edit_entry = None
            self.edit_item = None
            return
        ...
        self.table.item(self.edit_item, values=item_values)
    
        # Сохраняем в базу данных
        self.save_inline_to_db(item_values)
    ...
    def validate_inline_edit(self, value, col_index):
        """Проверяет значение перед сохранением"""
        if not value:
            return False
    
        # Для таблицы teachers
        if self.current_table == "teachers":
            if col_index in [1, 2, 3, 4, 5]:  # Фамилия, Имя, Отчество, Степень, Должность
                if any(char.isdigit() for char in value):
                    return False
                if len(value) < 2:
                    return False
            elif col_index == 6:  # Стаж
                try:
                    exp = int(value)
                    if exp < 0 or exp > 100:
                        return False
        ...
        # Для таблицы subjects
            ...
            if col_index == 1:  # Название
                if len(value) < 2 or len(value) > 70:
                    return False
            elif col_index == 2:  # Часы
                try:
                    hours = int(value)
                    if hours <= 0 or hours > 1000:
                        return False
        ...
        # Для таблицы workload
        elif self.current_table == "workload":
            if col_index == 3:  # Группа
                if len(value) < 1:
                    return False
    
        return True

    def save_inline_to_db(self, values):
        """Сохраняет изменения в базу данных"""
        record_id = values[0]
    
        if self.current_table == "teachers":
            database.update_teacher(
                self.database_file, int(record_id),
            ...
            # Для workload нужно получить ID преподавателя и предмета по имени
            teachers = database.get_all_teachers(self.database_file)
            subjects = database.get_all_subjects(self.database_file)
        
            teacher_id = next((t[0] for t in teachers if f"{t[1]} {t[2]} {t[3]}" == values[1]), None)
            subject_id = next((s[0] for s in subjects if s[1] == values[2]), None)
        
            if teacher_id and subject_id:
                database.update_workload(
                    self.database_file, int(record_id),
                    teacher_id, subject_id, values[3]
                )
